...
def train_ubal(dataset, epochs, arch):
    model = MyUBal(inp=arch[0], hid=arch[1], out=arch[2], betas=betas, gammasF=gammasF, gammasB=gammasB, alpha=alpha, testData=dataset)
...
if __name__ == "__main__":
    ...
    # Convert weightsF and weightsB to numpy arrays
    # Print the shape of each element in weightsF
    """for w in ubal_net.weightsF:
        print("Shape of weightsF element:", np.array(w).shape)
    weightsF_array = np.array([np.array(w) for w in ubal_net.weightsF])
    weightsB_array = np.array([np.array(w) for w in ubal_net.weightsB])"""
    np.savez("data/weights/trained_new_{}_{}.npz".format(trainingName, time.time()), wtsF=ubal_net.weightsF, wtsB=ubal_net.weightsB)
    #np.savez("data/weights/trained_new_{}_{}.npz".format(trainingName, time.time()), wtsF=weightsF_array, wtsB=weightsB_array)
