...
def newSom(rows, cols, inputsData, mask):
    ...
    model.train(inputsData, metric=metric, alpha_s=0.5, alpha_f=0.01, lambda_s=lambda_s,
                lambda_f=1, eps=100, trace=False, trace_interval=10)
    return model
...
if __name__ == "__main__":
    ...
    """
    Generate = True
    limbType = "left_hand"  #  limbType mozem tu definovat inak mi to nejde
    if Generate:
        inputs = genTouch()
    else:
        limbType = "left_hand"
        inputs = getRealTouchData("../data/data/data_matej_new.data", limbType).T
