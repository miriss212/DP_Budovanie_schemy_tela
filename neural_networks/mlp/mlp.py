...
class Mlp():
    ...
    def save(self, fileName):
        ...
        for i in range(len(self.theta_weights)):
            np.savetxt(fileName + "/" + str(i) + ".txt", self.theta_weights[i])

    def load(self, fileName):
        for i in range(len(self.theta_weights)):
            self.theta_weights[i] = np.loadtxt(fileName + "/" + str(i) + ".txt")
    ...
    def train(self, X, Y, iterations=400, reset=False):
        ...
        for iteration in range(iterations):
            print("EP: ", str(iteration), "/", str(iterations))
            ...
            for start, end in zip(X, Y):
                prediction = self.predict(np.array([start]))
                ...
                if np.all(roundPrediction == end):
                    ...
                    miss += 1
            classErr.append(((miss/n) * 100, miss))
            REs.append(re)
        return list(zip(REs, classErr))
