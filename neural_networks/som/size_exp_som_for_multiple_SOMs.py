...
def plot_winning_neurons_histograms(min_rows, max_rows, min_cols, max_cols, data_folder, out_name):
    ...
    total_plots = len(soms_and_errors)
    num_rows_required = (total_plots + num_cols - 1) // num_cols

    fig, axes = plt.subplots(num_rows_required, num_cols, figsize=(3 * num_cols, 2.5 * num_rows_required))
    axes = axes.flatten()
    ...
    plt.tight_layout()
    plt.savefig(f"winner_histogram_{out_name}_{time.time()}.pdf", format="pdf")
    plt.show()
    ...
    for rows in range(min_rows, max_rows + 1):
        for cols in range(min_cols, max_cols + 1):
            for k in range(5):
                ...
                experiment_folder = os.path.join(data_folder, f'SOM_{rows}x{cols}x{k}')
                ...
                som_saver = SOMSaver()
                som_saver.save_som(som=som, name=f'size_exp/SOM_{rows}x{cols}x{k}')

                # Store SOM and quantization error in the dictionary
                soms_and_errors[f'SOM_{rows}x{cols}x{k}'] = {
                    "som": som,
