...
class MyUBal:
    ...
    def train_minimal(self, data, max_epoch=None):
        ...
        for e in range(max_epoch):
            matches_train = 0
            matches_test = 0
            indexer = np.random.permutation(len(data))
            for i in indexer:
                X = np.expand_dims(np.transpose(data[i][0]), axis=1)
                y = np.expand_dims(np.transpose(data[i][1]), axis=1)
            ...
            acc_train_epoch.append(matches_train / len(data) * 100)
            #classification_errors_train.append(100 - acc_train_epoch[e])
            test_epoch_err, len_test = self.test()
            #test_epoch = (1 - test_epoch_err)*100
            test_epoch =  (len_test - test_epoch_err) / len_test * 100
            acc_test_epoch.append(test_epoch)

            if e == 0 or (e+1) % 5 == 0:
                print("Ep {} acc train: {:.3f}%".format(e+1, acc_train_epoch[e]))
        return self.network, acc_train_epoch, acc_test_epoch
