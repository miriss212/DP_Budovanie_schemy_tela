...
def train_ubal_crossval(dataset, epochs, folds=5, repetitions=5, hidden_layer=200, alpha = 0.6, betas=[0.0, 1.0, 0.0],
        ...
        for train_index, test_index in kfold.split(dataset):
            ...
            for X, y in data_train:
                ...
                if not np.all(targ_winners == pred_winners):
                    err_train += 1
            trainingAcc = 100 - ((err_train / len(data_train)) * 100)
...
if __name__ == "__main__":
    ...
    for body_part in body_parts_names:
        ...
        with open("../ubal_data/ubal_data_correct_touch/" + body_part + suffix) as f:
            data = json.load(f)
        k = 13
        # apply kwta per body part
        labeled = [(du.kwta_per_bodypart(np.array(i['pos']),k=k,continuous=True), np.array(i["touch"])) for i in data]
        acc_train_list, acc_test_list = train_ubal_crossval(labeled, hidden_layer=200, epochs=120)
